...
class PrivateCoinSettingController extends BaseController
{
    public function index(){
        $res['setting']=$this->get_info();
        $res['kind']=$this->keyVal();

        return view('PrivateCoinSetting.index',['res'=>$res]);
    }
    public function update(Request $request){
        $id=$request->id;
        $res=$this->get_single_info($id);
        $res['id']=$id;
        return view('PrivateCoinSetting.update',['res'=>$res]);
    }
    ...
    public function save(Request $request){
        ...
        try{
            Redis::hSet('GamePrivateCoinSetting#'.$id,'PrivateCoinRoom',$info);

            parent::saveLog('更新私房配置');
            exit(json_encode(['status'=>1,'msg'=>'ok','url'=>'/privateCoinSetting/index']));
        }catch (\Exception $e){
...
//        $arr[2]='1003';
    ...
    public function get_info(){
        $game=$this->games();
        foreach ($game as $k){
            $info=Redis::hGet('GamePrivateCoinSetting#'.$k,'PrivateCoinRoom');
    ...
    public function get_single_info($item){
        $info=Redis::hGet('GamePrivateCoinSetting#'.$item,'PrivateCoinRoom');

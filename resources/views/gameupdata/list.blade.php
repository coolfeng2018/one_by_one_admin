            ...
            <div class="portlet-body">
                <div class="table-toolbar">
                    <div class="row">
                        <div class="col-md-12">
                            <div class="btn-group">
                                <a class="btn green ajaxify" href="/gameupdata/addshow"> 添 加
                        ...
                        <table class="table table-striped table-hover table-bordered dataTable no-footer" id="sample_editable_1" role="grid" aria-describedby="sample_editable_1_info">
                            ...
                            @foreach($data as $k=>$v)
                                <tr role="row" @if($k%2==0) class="odd" @else class="even" @endif >
                                    ...
                                    <td> @if($v->update_type) 热更 @else apk更新 @endif </td>
                                    <td> @if ($v->allow_version=='*') 所有版本 @else {{$v->allow_version}} @endif</td>
                                    <td> @if ($v->allow_channel=='*') 所有渠道 @else {{$v->allow_channel}} @endif</td>
                                    <td>
                                        <a class="edit" href="javascript:Layout.loadAjaxContent('/gameupdata/edit?id={{ $v->id }}');"> 编辑 </a>
                                    </td>
                                    <td>
                                        <a class="delete" href="javascript:Layout.loadAjaxContent('/gameupdata/del?id={{ $v->id }}&vid={{ $v->id }}')"> 删除 </a>
                                    </td>
                                    <td>
                                        @if($v->is_public == "*") 是 @else 否 @endif </td>
                                    </td>
                                </tr>
                            @endforeach
                            </tbody>

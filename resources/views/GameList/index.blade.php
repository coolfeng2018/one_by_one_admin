            ...
            <div class="portlet-body">
                    ...
                    <div id="sample_1_wrapper" class="dataTables_wrapper no-footer">
                        <div class="row">
                            <div class="  input-inline ">
                                <a class="ajaxify btn sbold green" href="/gamelist/add"> 新增
                ...
                <table id="table" class="table table-striped table-bordered table-hover table-checkable order-column" style="word-break:break-all; word-wrap:break-all;table-layout:fixed">
                    ...
                    <tr>
                        ...
                        <th> 
                            <input type="checkbox"  name="check_s" onclick="check_s()" value="icon" id="server_icon">服务端<br>
                            <input type="checkbox"  name="check_c" onclick="check_c()" value="icon" id="customer_icon">客户端 
                        </th>
                        <th> 
                            <input type="checkbox"  name="check_s" onclick="check_s()" value="shown_type" id="server_shown_type">服务端<br>
                            <input type="checkbox"  name="check_c" onclick="check_c()" value="shown_type" id="customer_shown_type">客户端 </th>
                        <th> 
                            <input type="checkbox"  name="check_s" onclick="check_s()" value="status" id="server_status">服务端<br>
                            <input type="checkbox"  name="check_c" onclick="check_c()" value="status" id="customer_status">客户端 
                        </th>
                        <th>  
                    ...
                    <div>
                        ...
                        @foreach ($res as $resources)
                            <tr class="odd gradeX">
                                ...
                                <td>
                                    @if($resources->icon)
                                        <img src="{{config('suit.ImgRemoteUrl').$resources->icon}}" style="width:60px;height: 60px;">
                                    @endif
                                </td>
                                <td>
                                    @if($resources->shown_type==1)
                                        主界面
                                    @elseif($resources->shown_type==2)
                                        私人房
                                    @elseif($resources->shown_type==3)
                                        快速开始
                                    @endif
                                </td>
                                <td>
                                ...
                                <td>
                                    <a class="ajaxify btn sbold green" href="/gamelist/edit?GameListId={{ $resources->GameListId }}"> 修改
                                        <i class="fa fa-pencil"></i>
                                    </a>
                                    <a class="ajaxify btn sbold green" href="/gamelist/delete?GameListId={{ $resources->GameListId }}"> 删除
...
function verify(type){
    ...
    if(type=='server'){
        ...
        alert_array['id']="连续签到天数";
        alert_array['game_type']="游戏类型";
        alert_array['name']="游戏名";
        var must_array = ["id", "game_type", "name"];
        ...
        //必填项验证end
        url = '/gamelist/lua?select='+select+'&type=server';
    }else{
        var select = $("#check_customer_data").val();
        if(select==''){
            alert('请至少选择一项');return false;
        }
        url = '/gamelist/lua?type=customer&select='+select+'&type=customer';
    }
    //异步加载，返回数据拼装
    $.ajax( {
        type : "get",
